...
function contactSize(c: Contact): number {
  let s = 4;
  s += Math.min(c.interactions.length * 0.4, 3);
  s += Math.min(c.linkedProjects.length * 1, 3);
...
function projectSize(p: Project): number {
  let s = 5;
  s += Math.min(p.keyContacts.length * 0.6, 3);
  s += Math.min((p.meetingNotes?.length ?? 0) * 0.4, 2);
  if (p.status === 'active') s += 1.5;
  if (p.health === 'red') s += 1;
  return s;
}
...
function clientSize(c: Client): number {
  let s = 4.5;
  if (c.contractValue > 0) s += Math.min(Math.log10(c.contractValue / 100), 3);
  if (c.status === 'active') s += 1.5;
  return s;
}
